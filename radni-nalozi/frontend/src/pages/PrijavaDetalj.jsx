...
export default function PrijavaDetalj() {
  ...
  if (greska) return <Layout naslov="Prijava" nazad={true}><div className="greska">{greska}</div></Layout>
  if (!p) return <Layout naslov="Prijava" nazad={true}><Spinner /></Layout>
  ...
  return (
    <Layout naslov="Prijava kvara" nazad={true}>
      <div className="karta">
        <div className="naslov-red">
          <h3>{voziloLabel(p.vozilo)}</h3>
          <Bedz vrsta={p.status} tekst={STATUS_PRIJAVA[p.status]} />
        </div>
        <p className="meta"><Bedz vrsta={p.hitnost} tekst={'Hitnost: ' + HITNOST[p.hitnost]} /></p>
